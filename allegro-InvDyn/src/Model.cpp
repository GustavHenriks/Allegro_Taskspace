...
void allegro_air_uforce(double t,
                    double * q,
                    double * u) { };

void allegro_air_umotion(double t,
                    double * q,
                    double * u) { };
...
{
    allegro_air_state( tt, &in_state[0], &in_stated[0] );
}

Cvector Model::get_equivht()
{
    Cvector tau = Cvector::Zero(AIR_N_U);
    allegro_air_equivht( &tau[0] );
    return tau;
}
...
{
    Cvector tau = Cvector::Zero(AIR_N_U);
    allegro_air_comptrq( &udot[0],  &tau[0] );
    return tau;
}
...
{
    Cvector tau = Cvector::Zero(AIR_N_U);
    allegro_air_fulltrq( &udot[0], &mult[0], &tau[0] );
    return tau;
}
...
{
    Cvector tau = Cvector::Zero(AIR_N_U);
    allegro_air_multtrq( &mult[0], &tau[0] );
    return tau;
}
...
Cvector3 Model::get_pos(int body, Cvector3 pt)
{
    ...
    allegro_air_pos(body, &pt[0], &loc_cp[0]);
    return loc_cp;
}
...
Cvector3 Model::get_vel(int body, Cvector3 pt)
{
    ...
    Cvector3 loc_cp = zero_v3;
    allegro_air_vel(body, &pt[0], &loc_cp[0]);
    return loc_cp;
}
...
{
    Cvector3 loc_cp = zero_v3;
    allegro_air_acc(body, &pt[0], &loc_cp[0]);
    return loc_cp;
}
...
{
    double dircos[3][3];
    allegro_air_orient(body, dircos);
...
Cvector3 Model::get_angvel(int body)
{
    ...
    Cvector3 loc_cp;
    allegro_air_angvel(body, &loc_cp[0]);
	allegro_air_trans(body, &loc_cp[0], -1, &loc_cp[0]);
    return loc_cp;
}
...
Cvector3 Model::get_angacc(int body)
{
    // gives the velocity in the local ref frame, we should multiply by body
    Cvector3 loc_cp;
    allegro_air_angacc(body, &loc_cp[0]);
	allegro_air_trans(body, &loc_cp[0], -1, &loc_cp[0]);
    return loc_cp;
}
...
{
    double loc_cp[3];
    allegro_air_trans(frbod, &ivec[0], tobod, loc_cp);
...
Cmatrix Model::get_jacob(int body, Cvector3 pt, constraint_type type)
{
    ...
    for(int i=0;i<AIR_N_U;i++)
    {
        allegro_air_rel2cart(i, body, &pt[0], lin, rot);
        if(type==CT_TRANSLATION || type==CT_FULL)
        {
            allegro_air_trans(body, lin, -1, lin);
            J(0, i) = lin[0];
            J(1, i) = lin[1];
            J(2, i) = lin[2];
        }
        if(type==CT_ROTATION || type==CT_FULL)
        {
            ...
            allegro_air_trans(body, rot, -1, rot);
            J(shift+0, i) = rot[0];
            J(shift+1, i) = rot[1];
            J(shift+2, i) = rot[2];
        }
...
{
    allegro_air_setudot(&acc[0]);
}

void  Model::get_mom(Cvector3 &LM, Cvector3 &AM, double * KE)
{
    double ke;
    allegro_air_mom(&LM[0], &AM[0], &ke);
    if(KE!=NULL)
        *KE = ke;
}
...
Cvector3 Model::get_cm()
{
    ...
    for(int i=0;i<AIR_N_BODY;i++)
    {
        allegro_air_getmass(i, &mass);
        allegro_air_pos(i, pt_cp, loc_cp);
        cm[0] +=  loc_cp[0] * mass;
        cm[1] +=  loc_cp[1] * mass;
        cm[2] +=  loc_cp[2] * mass;
    }
...
Cvector3 Model::get_cm_v()
{
    ...
    {
        allegro_air_getmass(i, &mass);
        allegro_air_vel(i, pt_cp, loc_cp);
        cm[0] +=  loc_cp[0] * mass;
        cm[1] +=  loc_cp[1] * mass;
        cm[2] +=  loc_cp[2] * mass;
    }
...
Cvector3 Model::get_cm_a()
{
    ...
    {
        allegro_air_getmass(i, &mass);
        allegro_air_acc(i, pt_cp, loc_cp);
        cm[0] +=  loc_cp[0] * mass;
        cm[1] +=  loc_cp[1] * mass;
        cm[2] +=  loc_cp[2] * mass;
    }
...
Cmatrix Model::get_cm_J()
{
    ...
    {
        allegro_air_getmass(j, &mass);
        mass /= M;
        for(int i=0;i<AIR_N_U;i++)
        {
            allegro_air_rel2cart(i, j, pt_cp, lin, rot);
			allegro_air_trans(j, lin, -1, lin);
            J(0, i) += lin[0]*mass;
            J(1, i) += lin[1]*mass;
            J(2, i) += lin[2]*mass;
        }
...
{
    double dc_cp[3][3];
    allegro_air_quat2dc(st[0],st[1],st[2],st[3],dc_cp);
...
Cvector3 dc2ang(Cmatrix dc)
{
    ...
    allegro_air_dc2ang(dc_cp,&stang[0],&stang[1],&stang[2]);
    return stang;
}
...
{
    double dc_cp[3][3];
    allegro_air_ang2dc(stang[0],stang[1],stang[2],dc_cp);
    return matrix3_convert(dc_cp);
}
...
Cvector dc2quat(Cmatrix dc)
{
    ...
    double dc_cp[3][3];
    matrix3_convert(dc,dc_cp);
    allegro_air_dc2quat(dc_cp,&st[0],&st[1],&st[2],&st[3]);
    return st;
}
